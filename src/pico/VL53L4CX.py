...
class VL53L4CX:
    ...
    def _write_register(self, mem_address, data, length=None):
        if length is None:
            length = len(data)
        #with self.i2c_device as i2c:
        #    i2c.write(struct.pack(">H", address) + data[:length])
        self._i2c.writeto(self.address, struct.pack(">H", mem_address) + data[:length])

    def _read_register(self, mem_address, length=1):
        data = bytearray(length)
        #with self.i2c_device as i2c:
        #    i2c.write(struct.pack(">H", address))
        #    i2c.readinto(data)
        self._i2c.writeto(self.address, struct.pack(">H", mem_address), True)
