...
// Takes a single argument (the number to find the VL53L7CX of)
...
static mp_int_t VL53L7CX_helper(mp_int_t x)
{
    if (x == 0)
    {
        return 1;
    }
    return x * VL53L7CX_helper(x - 1);
}
...
static mp_obj_t VL53L7CX_calculate(mp_obj_t self_in)
{
    ...
    return mp_obj_new_int(VL53L7CX_helper(self->n));
}
