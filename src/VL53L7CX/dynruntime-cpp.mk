# Makefile fragment for generating native .mpy files from C/C++ source
...
CXXFLAGS += -DNO_QSTR
CXXFLAGS += -DMICROPY_ENABLE_DYNRUNTIME
CXXFLAGS += -DMP_CONFIGFILE='<$(CONFIG_H)>'
...
SRC_O += $(addprefix $(BUILD)/, $(patsubst %.c,%.o,$(filter %.c,$(SRC))) $(patsubst %.cpp,%.o,$(filter %.cpp,$(SRC))) $(patsubst %.S,%.o,$(filter %.S,$(SRC))))
...
CXXFLAGS += $(CFLAGS_ARCH) -DMICROPY_FLOAT_IMPL=MICROPY_FLOAT_IMPL_$(MICROPY_FLOAT_IMPL_UPPER)
...
# Build .o from .cpp source files
$(BUILD)/%.o: %.cpp $(CONFIG_H) Makefile
	$(ECHO) "CXX $<"
	$(Q)$(CROSS)g++ $(CXXFLAGS) -o $@ -c $<
